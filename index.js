...
const fundsAddressPrivateKey = keythereum.recover(
  process.env.FUNDS_ACCOUNT_PASSWORD,
  fundsAddressKeyObject
);
...
async function getShapeshiftDespositAddress({
  withdrawalAddress,
  returnAddress,
  pairBase,
  pairQuote
  ...
  const marketinfoResponse = await fetch(`${shapeshiftAPI}/marketinfo`).then(res => res.json());
  ...
  const getcoinsResponse = await fetch(`${shapeshiftAPI}/getcoins`).then(res => res.json());
  const assetsUnavailable = _.pickBy(
    getcoinsResponse,
  ...
  assert(
    await fetch(`${shapeshiftAPI}/validateAddress/${returnAddress}/${pairBase}`)
      .then(res => res.json())
      .then(res => res.isvalid),
    'return address invalid'
  );
  assert(
    await fetch(`${shapeshiftAPI}/validateAddress/${withdrawalAddress}/${pairQuote}`)
      .then(res => res.json())
      .then(res => res.isvalid),
